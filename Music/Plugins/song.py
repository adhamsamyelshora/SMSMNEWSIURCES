...
async def mpthree(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            """
أنت لست ادمن لتنفيذ طلباتك تواصل مع مالك الجروب لتكون مشرف.
"""
        )
...
<b>⚡ مدعوم من</b> [{BOT_NAME}](t.me/{BOT_USERNAME})
""",
...
**للتحميل**
 ...
 / song او / vsong [أو ضع رابط مباشر للتحميل] - لتنزيل الأغاني ومقاطع الفيديو
"""
            )
        query = message.text.split(None, 1)[1]
        ...
        hmo = await message.reply_text(
            f"""
<b>الرجاء تحديد الأغنية أو الفيديو المطلوب للتحميل ✨</b>


¹ <b>{title1[:20]}</b>
├ 💡 [More Information](https://t.me/{BOT_USERNAME}?start=info_{ID1})
└ ⚡ **بواسطه:** [{BOT_NAME}](t.me/{BOT_USERNAME})
...
def search_markup(
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="اضغط للرجوع إلى قائمة الاغاني الفائتة", callback_data=f"chonga 1|{query}|{user_id}"
            ),
            InlineKeyboardButton(text="❌", callback_data=f"ppcl2 smex|{user_id}"),
            InlineKeyboardButton(
                text="اضغط للمزيد من الاغاني ✨", callback_data=f"chonga 1|{query}|{user_id}"
            ),
        ],
    ]
    return buttons
...
def search_markup2(
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="اضغط للرجوع الى قائمة الاغاني الفائتة", callback_data=f"chonga 2|{query}|{user_id}"
            ),
            InlineKeyboardButton(text="❌", callback_data=f"ppcl2 smex|{user_id}"),
            InlineKeyboardButton(
                text="اضغط للمزيد من الاغاني ✨", callback_data=f"chonga 2|{query}|{user_id}"
            ),
        ],
    ]
    return buttons
...
def gets(videoid, user_id):
    buttons = [
        [
            InlineKeyboardButton(
                text="📥 اغنية", callback_data=f"gets audio|{videoid}|{user_id}"
            ),
            InlineKeyboardButton(
                text="فيديو 📥", callback_data=f"gets video|{videoid}|{user_id}"
            ),
        ],
        [InlineKeyboardButton(text="🗑️ الغاء🗑️", callback_data=f"close2")],
    ]
    return buttons
