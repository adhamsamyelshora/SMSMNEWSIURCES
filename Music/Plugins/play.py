...
@Client.on_message(command(["play", "شغل", "تشغيل", "سمسم" f"play@{BOT_USERNAME}", "p"]))
async def play(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            """
الرجاء استعمال حساب اساسي خاص بك
...
"""
    ...
    if a.status != "administrator":
        await message.reply_text(
            """
الاستعمال البوت احتاج المزيد من الصاليحات:

- ** يمكن إدارة الدردشة الصوتية: ** لإدارة الدردشة الصوتية
 - ** يمكن حذف الرسائل: ** لحذف الموسيقى غير المرغوب فيها التي تم البحث عنها
 - ** يمكن دعوة المستخدمين **: لدعوة المساعد للدردشة
...
"""
    ...
    if not a.can_invite_users:
        await message.reply_text(
            ...
            + "\n❌ قم بدعوة المستخدمين من خلال الرابط"
        )
        return
...
قم بإلغاء الحظر لأول مرة للاستخدام
"""
            )
            return
...
"""
    ...
    if audio:
        ...
        mystic = await message.reply_text(
            f"**جاري معالجة الصوت الان...✨ {username}**"
        )
...
<b>#️⃣ في قائمة الانتظار رقم</b> {position}
""",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
...
<b>في قائمة الانتظار رقم</b> {position}
""",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        os.remove(thumb)
...
async def popat(_,CallbackQuery): 
    ...
    if i == 2:
        buttons = search_markup(ID1, ID2, ID3, ID4, ID5, duration1, duration2, duration3, duration4, duration5, user_id, query)
        await CallbackQuery.edit_message_text(
            f"**✨ الرجاء تحديد الأغنية المطلوبة من أسفل ✨**¹ <b>{title1}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID1})__</u>\n\n² <b>{title2}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID2})__</u>\n\n³ <b>{title3}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID3})__</u>\n\n⁴ <b>{title4}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID4})__</u>\n\n⁵ <b>{title5}</b>\n  ┗  💡 <u>__[More Information](https://t.me/{BOT_USERNAME}?start=info_{ID5})__</u>",    
            reply_markup=InlineKeyboardMarkup(buttons),
            disable_web_page_preview=True 
        )  
        return
...
async def play_playlist_cmd(_, message):
    ...
    await message.reply_photo(
    photo=thumb, 
    caption=("**** تشغيل ميزة قائمة تشغيل الموسيقى \ حدد قائمة التشغيل التي تريد تشغيلها !."),    
    reply_markup=InlineKeyboardMarkup(buttons),
    )
    return
