...
async def stop_cmd(_, message): 
    ...
    if await is_active_chat(chat_id):
        ...
        await remove_active_chat(chat_id)
        await music.pytgcalls.leave_group_call(chat_id)
        await message.reply_text(f"**🎧 تم إيقاف تشغيل الموسيقى الآن يا {checking}!**") 
    else:
        return await message.reply_text("لا يوجد شئ مشغل")
...
async def stop_cmd(_, message): 
    ...
    if not await is_active_chat(chat_id):
        await message.reply_text("تم تخطي الأغنية يا")
    else:
        task_done(chat_id)
        if is_empty(chat_id):
            await remove_active_chat(chat_id)
